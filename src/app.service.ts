  ...
  async login (data : RegisterUserDTO) {
    try {
      ...
      const user = await this.prisma.user.findFirst({
        ...
        if (user == null) throw new NotFoundException("usernamenya salah icik boss")
          if (compareSync(data.password, user.password) == false) throw new BadRequestException ("password salah icik boss")
